...
import { getEvent, updateEvent, publishEvent } from "@/lib/api/events";
import { cn } from "@/lib/utils";
import type { UpdateEventRequest, EventVisibility } from "@/types/event";
...
// Form validation schema (similar to create, but dates can be in the past for existing events)
...
export default function EditEventPage() {
  ...
  const updateMutation = useMutation({
    mutationFn: async (data: { eventData: UpdateEventRequest; publish: boolean }) => {
      const response = await updateEvent(eventId, data.eventData);
      if (data.publish && event?.status === "draft") {
    ...
    onSuccess: (_, variables) => {
      ...
      toast.success(
        variables.publish && event?.status === "draft"
  ...
  // Loading state
  if (isLoading) {
    return <PageLoading message="Loading event..." />;
  }
  ...
  if (error || !event) {
    return (
      <ErrorState
        ...
        message={error?.message || "The event you're trying to edit doesn't exist."}
        onRetry={() => refetch()}
      />
    );
  }

  // Check if user can edit (host only)
      ...
      <ErrorState
        ...
        message={
          event.status === "cancelled"
            ...
            : "This event has completed and cannot be edited."
        }
      />
    );
  }
  ...
  return (
    <div className="space-y-6">
      ...
      {event.status === "draft" && (
        <div className="rounded-lg bg-amber-50 border border-amber-200 p-4 text-amber-800 dark:bg-amber-900/20 dark:border-amber-900 dark:text-amber-200">
          <p className="text-sm">
            <strong>Draft Event:</strong> This event is not yet published. You can publish it after saving your changes.
          </p>
        </div>
      )}
      ...
      <form className="space-y-6">
        ...
        <div className="flex flex-col-reverse gap-3 sm:flex-row sm:justify-end">
          ...
          >
            {updateMutation.isPending && !shouldPublish ? (
